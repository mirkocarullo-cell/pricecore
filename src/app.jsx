...
export default function PriceCore() {
  ...
  const valuta = async () => {
    ...
    const prompt = `Valuta questo dispositivo usato cercando online i prezzi reali attuali sul mercato italiano.
Dati: Modello: ${modello}, Storage: ${gb}, Batteria: ${batt}, Schermo: ${schermo}, Connettore: ${conn}, Altri danni: ${danniStr}, Acquistato: ${acq || "Nuovo"}.
Rispondi SOLO con questo JSON (valori in euro, usa RANGE min–max es. "320–380 €"):
{"modello":"nome completo","anno":"anno uscita","storage":"${gb}","valore_nuovo":"prezzo da nuovo €","valore_grado_a":"range usato grado A es. 320–380 €","valore_tuo":"range stimato per questo dispositivo es. 250–300 €","grado_stimato":"A o B o C","motivazione_grado":"2 righe","deprezzamenti":["motivo con impatto €"],"punti_forza":["punto"],"consigli_vendita":"2-3 consigli","copy_annuncio":"4 righe italiano persuasivo","fonti":["fonte/sito da cui derivano i prezzi"]}`;
    ...
    try {
      const res = await fetch("/api/valuta", {
        ...
        body: JSON.stringify({
          prompt,
          dati: { modello, gb, batt, schermo, conn, danni: danniStr, acq: acq || "Nuovo" },
        }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error.message || "Risposta non valida");
      const text = (data.content || []).filter(b => b.type === "text").map(b => b.text).join("\n");
      const json = text.slice(text.indexOf("{"), text.lastIndexOf("}") + 1);
      if (!json) throw new Error("Nessuna valutazione ricevuta");
            ...
            {result && (() => {
              ...
              return (
                <div>
                  ...
                  {d.fonti && d.fonti.length > 0 && (
                    <Card borderColor="#1a2a1a">
                      <SecTitle color="#7aa">🌐 Fonti prezzi (mercato reale)</SecTitle>
                      {d.fonti.map((x, i) => <Item key={i} color="#4caf50">{x}</Item>)}
                    </Card>
                  )}
